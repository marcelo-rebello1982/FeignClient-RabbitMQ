...
public class MongoDBClient {
	...
	public MongoTemplate mongoTemplate() {
		...
		if (System.getenv("NOSQL_ST_ADMIN") != null) {
			username = new String(Base64.getDecoder().decode(System.getenv("NOSQL_ST_ADMIN")));
		}
		if (System.getenv("NOSQL_ST_ADMIN_PASS") != null) {
			password = new String(Base64.getDecoder().decode(System.getenv("NOSQL_ST_ADMIN_PASS")));
		}
		if (System.getenv("NOSQL_DATABASE") == null) {
			...
			try (InputStream in = new FileInputStream(resourceConnection)) {
				...
				database = replaceChars(new String(Base64.getDecoder().decode(properties.getProperty("NOSQL_DATABASE_ADMIN"))));
				username = replaceChars(new String(Base64.getDecoder().decode(properties.getProperty("NOSQL_ST_ADMIN"))));
				password = replaceChars(new String(Base64.getDecoder().decode(properties.getProperty("NOSQL_ST_ADMIN_PASS"))));
